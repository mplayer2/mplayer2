...
// otherwise, returns 0.
static int get_udp(MPContext *mpctx,int blocking, double *master_position)
{
    char mesg[100];
...
int udp_slave_sync(MPContext *mpctx)
{
    ...
    master_exited = get_udp(mpctx,0, &udp_master_position);
    ...
    while (!master_exited || (!timed_out && master_exited < 0)) {
        ...
        // a datagram.
        master_exited = get_udp(mpctx,1, &udp_master_position);
        if (master_exited < 0)
            timed_out = 1;
    }
